...
function isPromptTagLine(line) {
  const trimmed = line.trim();
  return /^<\/?[A-Za-z][\w:-]*>$/.test(trimmed) || /^<!--\s*OMX:[\w:-]+\s*-->$/.test(trimmed);
}
...
function sanitizeMermaidFlowchart(source) {
  const firstLine = source
    .split("\n")
    .map((line) => line.trim())
    .find(Boolean);
  if (!firstLine || !/^(flowchart|graph)\b/.test(firstLine)) {
    return source;
  }

  return source.replace(/\b([A-Za-z][\w-]*)\[([^\]\n"[]*[()][^\]\n"[]*)\]/g, (_match, id, label) => {
    const quotedLabel = label.replace(/\\/g, "\\\\").replace(/"/g, '\\"');
...
function renderMarkdownBlock(text) {
  ...
  if (frontmatterMatch) {
    return `<pre><code>${escapeHtml(frontmatterMatch[1])}</code></pre>\n${renderMarkdownBlock(frontmatterMatch[2])}`;
  }
  if (/^---\n[\s\S]*\n---$/.test(trimmed)) {
  ...
  while (index < lines.length) {
    ...
    if (lines[index].startsWith("```")) {
      ...
      if (normalizedLanguage === "mermaid") {
        parts.push(`<div class="mermaid">${escapeHtml(sanitizeMermaidFlowchart(codeLines.join("\n")))}</div>`);
        continue;
      }
    ...
    if (isPromptTagLine(lines[index])) {
      parts.push(`<div class="prompt-tag"><code>${escapeHtml(lines[index].trim())}</code></div>`);
      index += 1;
      continue;
    }
